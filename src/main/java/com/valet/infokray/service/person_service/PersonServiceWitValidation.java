...
public class PersonServiceWitValidation implements PersonService {
    ...
    @WithoutValidation private final PersonService personService;
    ...
    public List<PersonDTO> findByName(String name) throws IllegalArgumentException {
        if (name != null && name.length() > 4 && name.length() < 50) {
            ...
            throw new IllegalArgumentException(
                    "name.length() must be greater than 4 and less than or equal to 50!");
    }

    @Override
    public Person save(Person person) throws IllegalArgumentException {
        if (person != null) {
            if (person.getName() != null && person.getName().split(" ").length <= 4) {
        ...
        } else throw new IllegalArgumentException("Person don't must be equal null!");
    }
