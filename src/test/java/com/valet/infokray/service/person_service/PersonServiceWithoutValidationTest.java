...
class PersonServiceWithoutValidationTest {
    @Autowired PersonServiceWithoutValidation personService;
    ...
    @Test
    void findByName() {
        Mockito.when(personRepo.findByName("Name"))
                .thenReturn(
                        List.of(
                                Person.builder().name("Name").id(1L).build(),
                                Person.builder().name("Name").id(2L).build()));
    ...
    void save() {
        ...
        Mockito.when(personRepo.save(person))
                .thenReturn(new Person(1L, "Test", "Test", "Test@test"));
